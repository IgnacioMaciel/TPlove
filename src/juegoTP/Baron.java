...
public class Baron extends Carta {
	...
	public void activarEfecto(Jugador jugador, ArrayList<Jugador> listaJugadores, Mazo m,
		...
		if (jugadoresElegibles.size() == 0) {
			...
			opcion = JOptionPane.showOptionDialog(pantallaPartida, "Elija un jugador para comparar cartas",
		...
		comparacion = jugador.getManoDeCartas().getMano().get(0)
				.compararCarta(jugadorElegido.getManoDeCartas().getMano().get(0));

		JOptionPane.showMessageDialog(pantallaPartida,
				jugador.getNombre() + " tiene la carta " + jugador.getManoDeCartas().getMano().get(0).getNombre()
						+ " con puntaje " + jugador.getManoDeCartas().getMano().get(0).getPuntajeFuerza() + "\n"
						+ jugadorElegido.getNombre() + " tiene la carta "
						+ jugadorElegido.getManoDeCartas().getMano().get(0).getNombre() + " con puntaje "
						+ jugadorElegido.getManoDeCartas().getMano().get(0).getPuntajeFuerza());
		...
		if (comparacion > 0) { /// EL JUGADOR QUE TENGA MENOR NUMERO DE CARTA ES DECLARADO FUERA DE LA RONDA
			...
			JOptionPane.showMessageDialog(pantallaPartida,
					"Jugador " + jugadorElegido.getNombre() + " quedo Fuera de Ronda.");
